...
public class AcademyService {
    ...
    public PageDto<GroupDto> findGroupsDto(Integer page, Integer size) {
        if (page < 0) page = 0;
        PageDto<GroupDto> pageDto = new PageDto<>();
        Page<Group> all = groupRepository.findAll(PageRequest.of(page, size));
        ...
        if (page > pageDto.getTotalPages()) {
            page = pageDto.getTotalPages() - 1;
            all = groupRepository.findAll(PageRequest.of(page, size));
        }

        pageDto.setCurrentPage(page);
        pageDto.setList(all.stream().map(g -> new GroupDto(g.getId(), g.getName()))
                .collect(Collectors.toList()));
        return pageDto;
    }

    public PageDto<StudentDto> findStudentsDto(Integer page, Integer size) {
        if (page < 0) page = 0;
        ...
        all = studentRepository.findAll(PageRequest.of(page, size));
        ...
        pageDto.setCurrentPage(page);
        pageDto.setList(all.getContent().stream()
                .map(s -> new StudentDto(s.getId(), s.getFirstName(), s.getLastName(), s.getBirthDate(),
                        s.getGroup().getId(), s.getGroup().getName()))
                .collect(Collectors.toList()));
        return pageDto;
    ...
    public PageDto<TeacherDto> findTeachersDto(Integer page, Integer size) {
        if (page < 0) page = 0;
        ...
        all = teacherRepository.findAll(PageRequest.of(page, size));
